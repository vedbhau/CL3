...
def start_sever():
    ...
    daemon = Pyro4.Daemon()  # creates a Pyro daemon that waits for incoming remote calls.
    url = daemon.register(conacatenator)  # registers your object with the daemon and returns a URL, which acts as the address clients will use to find your object.

    print("Server URL: ",url)  

    print("Server is ready.")
    daemon.requestLoop()   # keeps the server running, listening for method calls.
...
def main():
    ...
    concatenator = Pyro4.Proxy(url)  #creates a proxy object that acts like StringConcatenator but is actually a remote reference.
